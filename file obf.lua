...
local PandaAuth = loadstring(game:HttpGet(('https://pandadevelopment.net/service_api/PandaLibV3/Main.lua')))()
local savekeysystem = loadstring(game:HttpGet('https://pandadevelopment.net/servicelib?service='..ServiceID..'&core='..LibType..'&param='..LibVersion))()
...
local function CheckKey()
	...
	if keycheck == false then
        ...
        local keyloader = readfile("Key_Yutohub.txt")
		if savekeysystem:ValidateKey(service_name,keytype) or savekeysystem:ValidateKey(service_name,tostring(keyloader))  then 
...
local function DiscordLink()
    setclipboard("discord.gg/yutohubv2community");
    local success = true
    if success then
        Yutohub:MakeNotification({
            Name = "Copy To clipboard",
